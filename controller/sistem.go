...
// AddKehadiran inserts a new kehadiran record in the database
func AddKehadiran(kehadiran Kehadiran) error {
    _, err := kehadiranCollection.InsertOne(context.Background(), kehadiran)
    return err
}

// UpdateKehadiran updates an existing kehadiran record in the database
func UpdateKehadiran(name, date, subject string, kehadiran Kehadiran) error {
    filter := bson.M{"name": name, "date": date, "subject": subject}
    update := bson.M{
        "$set": kehadiran,
    }
    _, err := kehadiranCollection.UpdateOne(context.Background(), filter, update)
    return err
}

// DeleteKehadiran deletes a kehadiran record from the database
func DeleteKehadiran(name, date, subject string) error {
    filter := bson.M{"name": name, "date": date, "subject": subject}
    _, err := kehadiranCollection.DeleteOne(context.Background(), filter)
    return err
}

// GetAllKehadiran retrieves all kehadiran records from the database
func GetAllKehadiran() ([]Kehadiran, error) {
    var kehadiranList []Kehadiran
    ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
    defer cancel()

    cursor, err := kehadiranCollection.Find(ctx, bson.M{})
    if err != nil {
        return nil, err
    }
    defer cursor.Close(ctx)
...
func GetKehadiran(w http.ResponseWriter, r *http.Request) {
    kehadiranList, err := model.GetAllKehadiran()
    if (err != nil) {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
